 ...
 * Scope Lock") Section 1's single responsibility: **given a [KnowledgeQuery],
 * retrieve the already-promoted, already-existing [KnowledgeRecord]s
 * matching it.** Nothing more.
 *
 * **One operation, deliberately.** [recall] reuses [KnowledgeQuery] and
 * [KnowledgeRecord] unchanged -- no new query type, no new result type, no
 * new field. `recall` is named distinctly from [KnowledgeStore.retrieve] so
 * a caller holding only a [KnowledgeSource] reference is never confused for
 * one holding a full [KnowledgeStore] reference, even though the underlying
 * behaviour is identical.
 *
 * **Expresses no retrieval algorithm of its own.** [KnowledgeQuery] states
 ...
 * [KnowledgeStore] (Contract Design Section 4.1) -- this interface does not
 ...
 * interface's.
 *
 * **Passive read boundary, never authority.** [KnowledgeSource] exposes
 ...
 * forgetting remain [KnowledgeStore]'s own, sole, unchanged responsibility,
 ...
 * list -- never an exception -- when nothing matches [KnowledgeQuery], the
 * same convention [KnowledgeStore.retrieve] itself already establishes and
 ...
 * interface must be a pure projection of [KnowledgeStore]'s own owned state
 * (Scope Lock Section 3, Governing Principle) -- it must never construct
 * a [KnowledgeRecord] itself, never mutate a record, and never call
 * anything equivalent to [KnowledgeStore.remember] or [KnowledgeStore.forget].
 * This interface is declared separately from [KnowledgeStore] specifically
 * so a caller holding only a [KnowledgeSource] reference is structurally
 ...
 * here.
 */
fun interface KnowledgeSource {
    suspend fun recall(query: KnowledgeQuery): List<KnowledgeRecord>
}
