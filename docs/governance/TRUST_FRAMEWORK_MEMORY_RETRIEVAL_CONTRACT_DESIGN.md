**Status:** Governance and design only. No Kotlin is implemented, proposed as a diff, or changed by this document. Neither `src/` nor `tests/` is touched. This document does not amend `docs/architecture/10-permission-engine.md` ("Chapter 10"), `docs/architecture/08-resource-registry.md` ("Chapter 8"), `docs/architecture/MEMORY_CORE_CONTRACT_DESIGN_ERRATA_004.md` ("Errata 004"), `docs/architecture/MEMORY_CORE_SCOPE_LOCK.md`, `docs/governance/PROGRAMME_3_KNOWLEDGE_MEMORY_CONTRACT_DESIGN_V2.md`, `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_POLICY_RULE_COLLISION_CLARIFICATION.md`, `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_RESOLUTION_DERIVATION_MECHANISM_CLARIFICATION.md`, `docs/governance/TRUST_FRAMEWORK_AUTHORIZATION_CONTEXT_CONTRACT_DESIGN.md`, `docs/architecture/TRUST_FRAMEWORK_AUTHORIZATION_PURPOSE_PROGRAMME.md`, or any other frozen or draft governance document — it reads them, cites them, and reasons from them. It does not modify the Parker Conversational Memory Bridge's own uncommitted Admission Unit implementation, does not use that implementation as authority for anything decided below, and does not weaken any existing Trust Framework guarantee to accommodate it. This update is a dependency-consolidation pass only — it does not reopen or redesign anything already settled by either Adopted Clarification, the Authorization Context Contract Design, or the Authorization Purpose Programme. Nothing is staged, committed, or pushed.
...
Authorities read fresh, in full or in every relevant section, for this document: `docs/architecture/parker-constitution.md`; `docs/architecture/09-trust-framework.md`; `docs/architecture/10-permission-engine.md` (Chapter 10, in full — noted below as draft, not yet frozen); `docs/architecture/08-resource-registry.md` (Chapter 8); `docs/architecture/MEMORY_CORE_SCOPE_LOCK.md`; `docs/architecture/MEMORY_CORE_CONTRACT_DESIGN_ERRATA_004.md` (Errata 004, in full); `docs/decisions/CDR-005_CONSTITUTIONAL_ADMISSION_OF_PERMISSIONENGINE_PROPOSAL_CLASSES.md`; `docs/governance/PROGRAMME_3_KNOWLEDGE_MEMORY_CONTRACT_DESIGN_V2.md`; `docs/governance/PROGRAMME_3_UNIT_9_SCOPE_LOCK_CLARIFICATION.md`; `docs/architecture/IMPLEMENTATION_GAPS.md` (Gap #30, Gap #54, re-read fresh — Gap #54's own status remains "Open," unchanged by anything this consolidation records); `docs/architecture/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_GATING_BLOCKER.md` (the originating blocker document, re-read fresh); `docs/reviews/CONVERSATIONAL_MEMORY_ADMISSION_COMPLETION_REVIEW.md` and its own Independent Constitutional Review (evidentiary record of the discovery only — not authority for this design, per this document's own Status). **Also read fresh for this consolidation pass**: `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_POLICY_RULE_COLLISION_CLARIFICATION.md` (Adopted, in full); `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_RESOLUTION_DERIVATION_MECHANISM_CLARIFICATION.md` (Adopted, in full); `docs/governance/TRUST_FRAMEWORK_AUTHORIZATION_CONTEXT_CONTRACT_DESIGN.md` (Adopted, in full — treated as authority for Section 22's own dependency statement, not reopened or re-derived here); `docs/architecture/TRUST_FRAMEWORK_AUTHORIZATION_PURPOSE_PROGRAMME.md` (accepted at architecture tier, in full). Production code read fresh, in full: `src/composition/PermissionFilteredMemoryRetrieval.kt`, `src/composition/PermissionGatedMemoryCore.kt`, `src/runtime/DefaultPermissionPolicy.kt`, `src/runtime/ActionMapper.kt`, `src/interfaces/ResourceRegistry.kt`, `src/contracts/Resource.kt`, `src/contracts/Permission.kt`, `src/runtime/InMemoryResourceRegistry.kt`, `src/runtime/DefaultKnowledgeRetrieval.kt`, `src/runtime/DefaultKnowledgeCandidateEvaluator.kt`, `src/composition/ParkerRuntime.kt`.
...
`EvidenceIntelligenceInputResolver`'s own currently-correct, deliberately fail-closed retrieval (`ParkerRuntimeEvidenceIntelligenceCompositionTest`'s own already-accepted "even a record that genuinely exists" test) must remain fail-closed after this blocker is resolved, unless a future, separate, explicitly-reasoned decision changes that — this document takes no position on whether Evidence Intelligence's own retrieval *should* eventually become permissive, and treats its current fail-closed behaviour as a guarantee to preserve, not an accident to correct as a side effect of an unrelated fix. **This guarantee is now additionally, explicitly protected by the Authorization Purpose dependency Section 22 states** — no policy-content decision may approve `memory.retrieve` until a governed dimension exists capable of distinguishing Evidence Intelligence's own purpose from any other consumer's.
...
5. **RESOLVED AT THE ARCHITECTURAL TIER, IMPLEMENTATION-PENDING — shared-decorator indistinguishability between Knowledge Submission's and Evidence Intelligence's own `memory.retrieve` calls.** Disclosed by the Resolution Derivation Mechanism Clarification's own Section 7: `EvidenceIntelligenceInputResolver` and `DefaultKnowledgeCandidateEvaluator` both call `MemoryRetrieval`'s direct-lookup methods on the same, single, shared `PermissionFilteredMemoryRetrieval` instance, via the identical verb phrase, with no consumer-identity concept carried anywhere in the request — so a future policy-content decision to approve `memory.retrieve` cannot, as the system is presently composed, avoid simultaneously approving `EvidenceIntelligenceInputResolver`'s own retrieval, directly threatening Section 14's own guarantee. This item named a genuinely new architectural decision neither prior Clarification, nor this Contract Design, itself made. **That architectural decision has now been made, at the conceptual/Contract-Design tier, by `docs/governance/TRUST_FRAMEWORK_AUTHORIZATION_CONTEXT_CONTRACT_DESIGN.md` (Adopted) and carried into a sequenced body of future work by `docs/architecture/TRUST_FRAMEWORK_AUTHORIZATION_PURPOSE_PROGRAMME.md`: a new, closed, governed Authorization Purpose dimension, additive to Principal, evaluated by the same single `DefaultPermissionPolicy`.** See Section 22 for the full consolidation. What remains is not a further architectural question — it is the built, evaluable existence of that already-named capability (carrier mechanism, vocabulary registration, consumer retrofit), which this Contract Design does not authorise and does not attempt.
...
**Superseded in part, and further updated by this consolidation.** The two mechanism-level prerequisites this section originally called for are complete: `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_POLICY_RULE_COLLISION_CLARIFICATION.md` (Adopted) resolved the policy-rule collision (Section 19, item 2) via a per-verb-phrase rule discriminator; `docs/governance/TRUST_FRAMEWORK_MEMORY_RETRIEVAL_RESOLUTION_DERIVATION_MECHANISM_CLARIFICATION.md` (Adopted) resolved the resolution-derivation mechanism (Section 19, item 1).

**The narrower prerequisite pass this section previously recommended for item 5 (shared-decorator indistinguishability) has also now been undertaken and completed at the conceptual/Contract-Design tier**, by `docs/governance/TRUST_FRAMEWORK_AUTHORIZATION_CONTEXT_CONTRACT_DESIGN.md` (Adopted) and `docs/architecture/TRUST_FRAMEWORK_AUTHORIZATION_PURPOSE_PROGRAMME.md` (accepted at architecture tier). See Section 22 for the full consolidation. **The next recommended step is not a further Contract Design for Gap #54 itself, but the Authorization Purpose Programme's own first step — its carrier/representation design pass (Programme Section 9) — which this document does not authorise and does not begin.**

**A Scope Lock covering only the mechanism-level work both Adopted Clarifications together authorise** (verb-phrase-specific rule matching, plus closed-set, no-Resource-required derivation for `memory.retrieve`/`memory.retrieve_document`, with no `PermissionPolicyRule` content approving either) **could lawfully begin on the evidence gathered here.** A Scope Lock or Implementation Plan step that also adds a rule approving `memory.retrieve`/`memory.retrieve_document` — the step that would actually fix Gap #54's own live symptom — may not yet lawfully begin, and **may not lawfully begin until Authorization Purpose exists as an evaluable Trust Framework capability**, per Section 22.

The Parker Conversational Memory Bridge's own second unit (conversational retrieval) should continue to wait, as already established, until Gap #54 is fully, safely resolved — which now visibly depends on the Authorization Purpose Programme's own delivery, not merely on further work within this Contract Design.

---

## 22. Authorization Purpose Dependency (Consolidation)

*Added following the adoption of `docs/governance/TRUST_FRAMEWORK_AUTHORIZATION_CONTEXT_CONTRACT_DESIGN.md` and the architecture-tier acceptance of `docs/architecture/TRUST_FRAMEWORK_AUTHORIZATION_PURPOSE_PROGRAMME.md`. This section consolidates their conclusions into this Contract Design. It does not reopen Section 16's own candidate analysis, Section 17's own mechanism-level conclusions, or either Adopted Clarification — nothing already settled by those documents is redecided here.*

**The architectural dependency, stated in general terms.** Memory Retrieval permission evaluation requires a governed Authorization Purpose dimension sufficient to distinguish constitutionally different internal purposes that otherwise share the same Principal, Action, and Resource classification. This is a general architectural requirement, not a requirement naming any one consumer: Section 19 item 5's own finding demonstrates that two independently-governed, differently-purposed consumers of the identical Memory Core retrieval boundary are structurally indistinguishable under Principal, Action, and Resource alone, however each of those three dimensions is derived or matched.

**Why Principal, Action, and Resource together remain insufficient.** Sections 6–8 already establish how Action and Resource identity are resolved for Memory Core retrieval; the Policy Rule Collision Clarification already extended Action-level matching as far as a verb-phrase discriminator can honestly go. None of that work, individually or combined, changes Section 19 item 5's own finding: `EvidenceIntelligenceInputResolver` and `DefaultKnowledgeCandidateEvaluator` propose the identical verb phrase, resolve to the identical `(action, resourceType)` pair, and each carries a Principal that is — correctly, per Errata 004 — the genuinely accountable one for its own respective caller, never a caller-distinguishing marker. **Gap #54 cannot be fully resolved through Principal, Action, and Resource alone**; a fourth, independent dimension is constitutionally required, exactly as the Authorization Context Contract Design's own audit concluded from first principles, independent of any caller-specific concern.

**What is settled, unchanged by this consolidation:**

- The policy-rule verb discrimination mechanism (Section 17, Section 19 item 2) — unchanged.
- The closed-set, no-Resource-required derivation mechanism for structurally empty `targetResources` (Section 17, Section 19 item 1) — unchanged.

**What was an unresolved architectural question specific to this Contract Design (Section 19 item 5) is now resolved at the conceptual/architectural tier** by the Authorization Context Contract Design (Adopted) and carried into a sequenced body of future work by the Authorization Purpose Programme (accepted at architecture tier). This document adopts their conclusion as its own governing dependency rather than repeating or re-deriving their reasoning.

**This document no longer implies that approving `memory.retrieve`/`memory.retrieve_document` can proceed before Authorization Purpose exists in the live permission model.** Any Scope Lock or Implementation Plan step that would add a `PermissionPolicyRule` approving either action name, before Authorization Purpose is an evaluable Trust Framework capability, would necessarily approve it identically for every consumer sharing that verb phrase — including `EvidenceIntelligenceInputResolver` — regardless of anything already decided at the mechanism level by the two Adopted Clarifications.

**Evidence Intelligence** must remain protected from accidental widening: no policy-content decision may approve `memory.retrieve` until an Authorization-Purpose-aware rule can distinguish Evidence Intelligence's own purpose from any other consumer's. This strengthens, and does not alter, Section 14's own existing guarantee.

**Knowledge Submission** must eventually be able to receive its own lawful Authorization Purpose without bypassing the Trust Framework — not by reverting to the ad hoc `system.knowledge-memory` identity substitution the Authorization Context Contract Design's own audit already found ungoverned and independently broken, and not by any caller-specific exception of the kind Section 18 already, permanently rejects.

**Future Conversational Retrieval** must use the same general Authorization Purpose mechanism every other Memory Core consumer uses, declaring its own registered purpose value through whatever carrier mechanism the Authorization Purpose Programme's own future work eventually selects — never a bespoke exception reserved for it. This reaffirms, unchanged, Section 15's and Section 21's own existing "same general terms as any other caller" position.

**Dependency sequencing.** The following chain is supported by the cited, accepted documents and is recorded here for continuity — a description of an already-established dependency order, not a new scheduling commitment. No step within it is authorised to begin by this document, and no carrier/API shape for Authorization Purpose is assumed or invented here:

```
Authorization Purpose Contract Design (Adopted)
        ↓
Authorization Purpose Programme (accepted, architecture tier)
        ↓
Authorization Purpose carrier/evaluation governance (Programme's own first step — not yet begun; carrier/API shape deliberately unresolved, per Programme Section 9)
        ↓
Gap #54 Scope Lock / implementation (mechanism-level work may begin now; a rule approving memory.retrieve may not, until the step above delivers an evaluable Authorization Purpose capability)
        ↓
Knowledge Submission live success (contingent on the still-separate, still-undecided policy-content decision, Section 17 — KnowledgeSubmission.submit resolving and promoting candidates in the live, composed runtime — NOT a guaranteed consequence of the steps above it)
        ↓
Conversational Memory Admission merge (IMPLEMENTATION_GAPS.md Gap #54 already records the Admission Unit as itself, directly, confirmed blocked by this same gap)
        ↓
Future Conversational Retrieval (the Bridge's own second unit, already established to wait on Gap #54's full resolution)
```

**This chain describes an order, not a guaranteed outcome.** Reaching the step immediately above "Knowledge Submission live success" does not, by itself, resolve Section 17's own still-open policy-content question — whether, and for whom, `memory.retrieve`/`memory.retrieve_document` should actually receive an `APPROVED` `PermissionPolicyRule` outcome. That decision remains genuinely undecided by every document in this governance chain, including this one.

**Whether this Contract Design is now constitutionally complete or still contains unresolved contract-level questions: constitutionally complete, and implementation-blocked.** Every architectural question this document exists to answer — what a Memory Core record is for permission-evaluation purposes (Section 6), how the `(action, resourceType)` pair is derived without a Resource (Section 17, item 1), how two verb phrases sharing that pair receive independent outcomes (Section 17, item 2), and what additional dimension is required to lawfully distinguish two consumers sharing an identical verb phrase (this section) — now has a settled, Contract-Design-tier answer; none of it is reopened or left as an open design question by this consolidation. What remains is not a further architectural question this document, or any sibling document in this governance chain, has yet to answer — it is the built, evaluable existence of the capability already named: Authorization Purpose's own carrier mechanism, vocabulary registration, and consumer retrofit (Authorization Purpose Programme, its own Section 10), followed by the still-separate, still-undecided policy-content question (Section 17: whether `memory.retrieve` should actually be approved once distinguishable) that was never this document's own responsibility to decide in the first place.

---

## 23. Independent Constitutional Review Self-Check
