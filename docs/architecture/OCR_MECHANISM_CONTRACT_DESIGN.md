...
- **Disclosing recognition fidelity.** Where recognition is uncertain, partial, or requires reconstructing an illegible or ambiguous passage, disclose which of the Evidence Intelligence Contract Design's own already-frozen fidelity categories (§5: verbatim transcription, normalised transcription, inferred reconstruction) the output — or which portion of it — represents. This document does not invent a fourth category; it binds this mechanism's own output to the three that already exist.
...
- **A fidelity disclosure**, binding the output to one of the Evidence Intelligence Contract Design's own three already-frozen categories (§5): verbatim transcription, normalised transcription, or inferred reconstruction — or, where more than one applies within a single recognition, which portions are which. This document introduces no fourth category.
