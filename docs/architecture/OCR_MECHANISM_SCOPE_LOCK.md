...
4. Every recognition the OCR mechanism discloses carries a fidelity classification drawn from the Evidence Intelligence Contract Design's own four governed categories (§5, as amended by `OCR_TRANSCRIPTION_FIDELITY_VERIFICATION_AMENDMENT.md`) — `VERBATIM`, `UNVERIFIED_LITERAL_TRANSCRIPTION`, `NORMALISED`, or `INFERRED_RECONSTRUCTION` — never a fifth, invented category. Ordinary machine recognition success is not independently established verbatim source fidelity.
