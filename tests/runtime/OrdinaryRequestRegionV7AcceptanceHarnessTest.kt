...
class OrdinaryRequestRegionV7AcceptanceHarnessTest {
    ...
    private fun verifyFrozen(dir:Path,f:Fixture,p:Prepared){val actual=objectField(readEnvelope(dir.resolve("manifest.json")),"record");require(RegionJson.encode(actual)==RegionJson.encode(manifest(f,p)));if(f.name=="B"){
        require(p.graphs.map{it.regions.size}==listOf(16,14,6));require(p.request.regions.groupingBy{it.pageNumber}.eachCount().toSortedMap().values.toList()==listOf(14,12,6));require(p.graphs.sumOf{it.regions.size}==36);require(p.request.regions.flatMap{it.constituents}.map{it.id}.distinct().size==36)}}
