...
class DefaultGovernedHumanCorrectionServiceTest {
    ...
    fun `exact R6 correction converges create-once and remains durable after restart`() = runTest {
        ...
        assertTrue(created.correctedTranscriptionBlocks[0].endsWith("Kellec"))
        assertTrue(created.correctedTranscriptionBlocks[4].endsWith("Kellec"))
        assertEquals(fixture.provider.transcriptionBlocks.slice(1..3), created.correctedTranscriptionBlocks.slice(1..3))
        assertTrue(fixture.provider.transcriptionBlocks[0].endsWith("Kellee"))
        assertTrue(fixture.provider.transcriptionBlocks[4].endsWith("Kellee"))
    ...
    private fun provider(review: HumanFidelityReviewRecord): OrdinaryRegionTranscriptionDerivative {
        val blocks = (1..5).map { "Execution page $it — Michael Gary Kellee" }
        return OrdinaryRegionTranscriptionDerivative(
            ...
            pageBindings=(1..5).map { "page-$it" }, regionBindings=(1..5).map { (it + 10).toString().padStart(64, '0') },
