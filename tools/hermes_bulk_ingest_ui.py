...
let batches=[], files=[];
async function ready(){let r=await fetch('/api/ready-batches',{headers:apiHeaders});let d=await r.json();batches=(d.batches||[]).filter(b=>b.status==='READY');let s=document.querySelector('#batch');s.innerHTML='';batches.forEach((b,i)=>{let o=document.createElement('option');o.value=i;o.textContent=b.caseName+' · '+b.batchId;s.appendChild(o)}); update()}
...
document.querySelector('#start').onclick=async()=>{let b=batches[document.querySelector('#batch').value],p=document.querySelector('#progress'),counts={PASS:0,REVIEW_REQUIRED:0,FAILED:0,ANALYSIS_READY:0,REQUIRES_OCR:0,CAPABILITY_UNAVAILABLE:0,BLOCKED:0},bad=[];if(!b||!confirm(`Process ${files.length} file(s) for Parker case “${b.caseName}”?`))return;for(let i=0;i<files.length;i++){let f=files[i];p.textContent=`File ${i+1} / ${files.length}: ${f.name}`;let form=new FormData();form.append('file',f,f.name);let r=await fetch('/api/ingest?batchId='+encodeURIComponent(b.batchId),{method:'POST',headers:apiHeaders,body:form});let d=await r.json();counts[d.status]=(counts[d.status]||0)+1;if(d.governed_ingestion)counts[d.governed_ingestion]=(counts[d.governed_ingestion]||0)+1;if(d.reason||d.detail)bad.push(f.name+': '+(d.reason||d.detail))}document.querySelector('#summary').textContent=`Case name: ${b.caseName}\nBatch ID: ${b.batchId}\nFiles discovered: ${files.length}\nPASS: ${counts.PASS}\nREVIEW_REQUIRED: ${counts.REVIEW_REQUIRED}\nFAILED: ${counts.FAILED}\nANALYSIS_READY: ${counts.ANALYSIS_READY}\nREQUIRES_OCR: ${counts.REQUIRES_OCR}\nCAPABILITY_UNAVAILABLE: ${counts.CAPABILITY_UNAVAILABLE}\nBlocked: ${counts.BLOCKED}`+(bad.length?'\n\nProblems:\n'+bad.join('\n'):'')};
ready();
</script>'''
    ...
    def do_POST(self):
        ...
        try:
            ...
            try:
                ...
                item = process_one(ParkerClient(os.environ.get("PARKER_GATEWAY_URL", "http://127.0.0.1:8090"), self.token, 120), batch, source_path, 120, filename)
